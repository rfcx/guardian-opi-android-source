...
//==========lovelyfonts end ===========
	...
	// Add by liyu 2014.11.05 for magic track
	private final static String KEY_MAGIC_TRACK_PREF = "oppo_magic_track";
	private ListPreference mMagicTrackPreference;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Add by liyu 2014.11.05 for magic mode
     	mMagicTrackPreference = (ListPreference) findPreference(KEY_MAGIC_TRACK_PREF);
     	if (mMagicTrackPreference != null) {
     		int mode = getMagicTrackMode();
     		mMagicTrackPreference.setValue(String.valueOf(mode));
     		int index = getMagicTrackIndex(mode);
     		mMagicTrackPreference
     			.setSummary(mMagicTrackPreference.getEntries()[index]);
     		mMagicTrackPreference.setOnPreferenceChangeListener(this);
     	}
     	// Add end
...
//==========lovelyfonts add ===========
        ...
        try {
            PackageInfo packageInfo = getPackageManager().getPackageInfo(
                    "com.lovelyfonts", 0);
            if (packageInfo != null) {
                getPreferenceScreen().addPreference(mFontSettingPreference);
            } else {
                mDisplaySettingsExt.removePreference(mFontSettingPreference);
            }
...
//==========lovelyfonts end ===========
	...
	// Add by liyu 2014.11.05
	private int getMagicTrackMode() {
		return Settings.System.getInt(getContentResolver(),
				"runyee_magic_track_mode", 0);
	}

	private int getMagicTrackIndex() {
		int mode = getMagicTrackMode();
		if (mMagicTrackPreference != null) {
			final CharSequence[] values = mMagicTrackPreference
					.getEntryValues();
	...
	private int getMagicTrackIndex(int mode) {
		if (mMagicTrackPreference != null) {
			final CharSequence[] values = mMagicTrackPreference
					.getEntryValues();
			for (int i = 0; i < values.length; i++) {
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        // Add by liyu 2014.11.05 for magic mode
        if (preference == mMagicTrackPreference) {
     		int mode = Integer.parseInt((String) objValue);
     		Settings.System.putInt(getContentResolver(),
     				"runyee_magic_track_mode", mode);
     		int index = getMagicTrackIndex(mode);
     		mMagicTrackPreference
     				.setSummary(mMagicTrackPreference.getEntries()[index]);
     	}
