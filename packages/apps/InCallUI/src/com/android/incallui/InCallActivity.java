...
import com.mediatek.incallui.NotifySTKDisconnect;
...
public class InCallActivity extends Activity {
    ...
    private boolean mDdelayShowErrorDialogRequest = false;
    private int mErrorDialogResID;

    private boolean mhasPendingWaitDialog = false;
    private int mPendingPostCharWaitDialogCallId;
    private String mPendingPostCharWaitDialogChars;
    ...
    protected void onResume() {
        ...
        /// M: For performance auto test filter @ {
        Log.pi("sera","[Performance test][Phone] Call_Performance_004 end [" + System.currentTimeMillis() + "]");
        /// M: @}

        ///M: make sure before the in call UI showing we clear all the existed dialogs, especially
        /// some dialogs shown out after onPause executed.
        ...
        /// M: Add for ALPS01469149. Show error dialog after activity resume @{
        if (mDdelayShowErrorDialogRequest){
            if (mErrorDialogResID != INVALID_RES_ID)
            showErrorDialog(mErrorDialogResID);
            mDdelayShowErrorDialogRequest = false;
        }
        /// @}
        ...
        /// M: for ALPS01487565. Show dialog after IncallActivity resume @{
        if (mhasPendingWaitDialog) {
            final PostCharDialogFragment fragment = new PostCharDialogFragment();
            fragment.setParameters(mPendingPostCharWaitDialogCallId, mPendingPostCharWaitDialogChars);
            fragment.show(getFragmentManager(), "postCharWait");
            mhasPendingWaitDialog = false;
            mPendingPostCharWaitDialogCallId = -1;
            mPendingPostCharWaitDialogChars = null;
        }
        /// @}
        /// M: For notify stk when call disconnect. @{
        NotifySTKDisconnect.getInstance().clearNotifyStkFlag();
        /// @}
    }
    ...
    protected void onPause() {
        ...
        if (proximitySensor != null) {
            proximitySensor.screenOffForSmartBook();
        }
        /** @} */
    ...
    protected void onStop() {
        ...
        /// M: For performance auto test filter @ {
        Log.pi("sera","[Performance test][Phone] Call_Performance_005 end [" + System.currentTimeMillis() + "]");
        /// M: @}
        ...
        /// M: For notify stk when call disconnect. @{
        NotifySTKDisconnect.getInstance().notifyStkCallDisconnected(getApplicationContext());
        /// @}
    }
    ...
    protected void onNewIntent(Intent intent) {
        ...
        ProximitySensor proximitySensor = InCallPresenter.getInstance().getProximitySensor();
        if (proximitySensor != null) {
            proximitySensor.lightOnScreenForSmartBook();
        }
        /// @}
    ...
    public void displayDialpad(boolean showDialpad) {
        if (showDialpad) {
            ...
            /**
             * M: in landscape, should not hide it
             * Original code:
    ...
    public void showPostCharWaitDialog(int callId, String chars) {
        if(isForegroundActivity()){
            final PostCharDialogFragment fragment = new PostCharDialogFragment();
            fragment.setParameters(callId, chars);
            fragment.show(getFragmentManager(), "postCharWait");
        }else{
            mhasPendingWaitDialog = true;
            mPendingPostCharWaitDialogCallId = callId;
            mPendingPostCharWaitDialogChars = chars;
        }
    }
    ...
    public void maybeShowErrorDialogOnDisconnect(Call.DisconnectCause cause) {
        ...
        if (!isFinishing()) {
            ...
            if (resId != INVALID_RES_ID) {
                /// M: Add for ALPS01469149
                //if InCallActivity has not resumed already, show error dialog later. @}
                if (!isResumed()){
                    Log.d(this, "maybeShowErrorDialogOnDisconnect, activity not resumed");
                    mDdelayShowErrorDialogRequest = true;
                    mErrorDialogResID = resId;
                    return;
                }
                /// @}
    ...
    private void initializeInCallMTK() {
        if (FeatureOptionWrapper.isSupportVT() && mVTCallFragment == null) {
            ...
            mVTCallFragment.setVTScreenMode(VTScreenMode.VT_SCREEN_CLOSE);
        }
        /// DM Lock Feature @ {
            ...
            oKClickListener = new DialogInterface.OnClickListener() {
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (null != mDialog) {
                        mDialog.dismiss();
                        /// M: For ALPS01498711. @{
                        // need to set mDialog as null, or else hasPendingErrorDialog() func will got error,
                        // because hasPendingErrorDialog fuc is just judge if mDialog is null.
                        mDialog = null;
                        /// @}
                    }
                    // To Setting Storage
