...
public class VoiceSearchManager {
    ...
    public VoiceSearchManager(Context context, Listener listener) {
        mContext = context;
        mListener = listener;
        ...
        mCallback = new IVoiceCommandListener.Stub() {
            ...
            public void onVoiceCommandNotified(int mainAction, int subAction, Bundle extraData) throws RemoteException {
                LogUtils
                        .i(TAG, "[onVoiceCommandNotified] [vcs]mainAction = " + mainAction + ", subAction=" + subAction);
                switch (mainAction) {
    ...
    public void startVoiceSearch(int orientation) {
        LogUtils.i(TAG, "[startVoiceSearch] [vcs] mVCmdMgrService:" + mVCmdMgrService);
        if (isVCmdMgrServiceExists() && !mIsRegistered) {
            LogUtils.i(TAG, "[startVoiceSearch] [vcs] startVoiceCommand orientation:" + orientation);
            startVoiceCommand(orientation);
        }
    }

    public void stopVoiceSearch() {
        if (mVCmdMgrService != null && mIsRegistered) {
            LogUtils.i(TAG, "[stopVoiceSearch] [vcs] stopVoiceCommand()");
            stopVoiceCommand();
        }
    }
    ...
    public void onSetScreenOrientation(int orientation) {
        ...
        LogUtils.i(TAG, "[onSetScreenDirection] [vcs] screen direction:"
    ...
    public void onContactLearning(String contactName) {
        ...
        LogUtils.i(TAG, "[onContactLearning] [vcs] study name:"
    ...
    private void startVoiceCommand(int orientation) {
        ...
        if (null == mContext) {
            LogUtils.i(TAG, "[startVoiceCommand] mContext is null, just return");
            return;
        }
        ...
        // TODO add voice command token for clear duplicated notify results.
        // mVoiceCommandToken++;
        // Bundle extraData = new Bundle();
        // LogUtils.i(TAG,
        // "[startVoiceCommand] [vcs] start mVoiceCommandToken:"+mVoiceCommandToken);
        // extraData.putInt(VoiceCommandListener.ACTION_EXTRA_RESULT_INFO,
        // mVoiceCommandToken);
        //
        final String pkgName = mContext.getPackageName();
    ...
    private void stopVoiceCommand() {
        ...
        if (null == mContext) {
            LogUtils.i(TAG, "[stopVoiceCommand] mContext is null, just return");
            return;
        }
    ...
    private void unRegisterVoiceCommand(String pkgName) {
        if (mVCmdMgrService != null) {
            try {
                ...
                mIsRegistered = false;
                mVCmdMgrService = null;
                LogUtils.i(TAG, "[unRegisterVoiceCommand] [vcs] Unregister error in handler RemoteException = "
                        + e.getMessage());
            }
        }
    ...
    private Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            case MSG_SEARCH_NOTIFY:
                ...
                if (nameArray == null) {
                    LogUtils.i(TAG, "[handleMessage] [vcs] nameArray is null");
                    return;
                } else if (nameArray.length == 0) {
                    LogUtils.i(TAG, "[handleMessage] [vcs] nameArray is empty");
                    mListener.onNoContactFoundInVoiceBank();
                    return;
                }
                // clear it before use
    ...
    private void handleContactsUIVoiceCommand(int subAction, Bundle extraData) {
        ...
        case VoiceCommandListener.ACTION_VOICE_CONTACTS_START:
            LogUtils.i(TAG, "[handleContactsUIVoiceCommand] [vcs] VoiceCommandListener.ACTION_VOICE_CONTACTS_START");
            // refers: means listenning begin.(Recoginze start)
            if (VoiceCommandListener.ACTION_EXTRA_RESULT_SUCCESS != extraData
                    .getInt(VoiceCommandListener.ACTION_EXTRA_RESULT)) {
                LogUtils.w(TAG, "[handleContactsUIVoiceCommand] ACTION_VOICE_CONTACTS_START failed..");
        ...
        case VoiceCommandListener.ACTION_VOICE_CONTACTS_STOP:
            LogUtils.i(TAG, "[handleContactsUIVoiceCommand] [vcs] VoiceCommandListener.ACTION_VOICE_CONTACTS_STOP");
            // refers: means listenning end.(Recoginze stop)
            break;
        ...
        case VoiceCommandListener.ACTION_VOICE_CONTACTS_NOTIFY:
            LogUtils.i(TAG, "[handleContactsUIVoiceCommand] [vcs] VoiceCommandListener.ACTION_VOICE_CONTACTS_NOTIFY");
            //
            if (VoiceCommandListener.ACTION_EXTRA_RESULT_SUCCESS != extraData
                    .getInt(VoiceCommandListener.ACTION_EXTRA_RESULT)) {
                LogUtils
