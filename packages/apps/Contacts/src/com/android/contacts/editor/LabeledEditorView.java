    ...
    protected void onTypeSelectionChange(int position) {
        ...
        if (cae.isFeatureAccount(accountType, ExtensionManager.COMMD_FOR_AAS)
            ...
            if (Phone.TYPE_CUSTOM == selected.rawValue) {
                ExtensionManager.getInstance().getLabeledEditorExtension().onTypeSelectionChange(selected.rawValue,ExtensionManager.COMMD_FOR_AAS);
                Log.i(TAG, "[onTypeSelectionChange] onTypeSelectionChange");
            } else {
                mType = selected;
                mEntry.put(mKind.typeColumn, mType.rawValue);
