...
public class PeopleActivity extends ContactsActivity
    ...
    protected void onCreate(Bundle savedState) {
        ...
        if (VCSUtils.isVCSFeatureEnabled()) {
            ...
            mVoiceSearchMgr = new VoiceSearchManager(getApplicationContext(), this);
...
//            if (!VCSUtils.isVCSFeatureEnabled()) {
//                LogUtils.i(TAG, "[onTouchedScreen] [vcs] not support VCS");
//                return;
//            }
//
//            int tab = mActionBarAdapter.getCurrentTab();
//            LogUtils.i(TAG, "[onPageScrollStateChanged] [vcs] pager state:" + state + ",tab:" + tab);
//            switch (state) {
//            case ViewPager.SCROLL_STATE_DRAGGING:
//                if (mVCSHandler.hasMessages(MSG_UPDATE_VCS_STATUS)) {
//                    mVCSHandler.removeMessages(MSG_UPDATE_VCS_STATUS);
//                }
//                LogUtils.i(TAG, "[onTouchedDown] [vcs] stopVoiceSearch. tab:" + tab);
//                stopVoiceService();
//                break;
//            case ViewPager.SCROLL_STATE_IDLE:
//                LogUtils.i(TAG, "[onTouchedUp] [vcs] delay 1s start voice search process. tab:" + tab);
//                if (tab == TabState.ALL) {
//                    Message msg = Message.obtain();
//                    msg.what = MSG_UPDATE_VCS_STATUS;
//                    mVCSHandler.sendMessageDelayed(msg, 300);
//                }
//                break;
//            default:
//                break;
        ...
        switch (item.getItemId()) {
            ...
            case R.id.menu_add_contact: {
                ...
                if (PhoneCapabilityTester.isUsingTwoPanes(this)) {
                    ...
                    // intent itself.
                    /**
                     * M: [ALPS01261710] error handling for seldom ActivityNotFound
                     * @{
                     */
                    try {
                    /** @} */
                     ...
                     */
                        ...
                        int state = getPackageManager().getComponentEnabledSetting(new ComponentName(this, ContactEditorActivity.class));
                        LogUtils.e(TAG, "[onOptionsMenuSelected]ContactEditorActivity state:" + state + ",e:" + e);
                        e.printStackTrace();
                    }
                    /** @} */
                }
...
//        }
    ...
    */
    ...
    private static final int TIME_CIRCLE_SHOULD_SHOW = 500;
    ...
    private boolean checkStartVcsCondition() {
        ...
        if ((!mIsShowingGuide) && isResumed() && (!isSearchMode) && (tab == TabState.ALL) && mContactsCount > 0) {
            return true;
        }
        return false;
    }
    ...
    public void onShowContactsCount(int count) {
        ...
        int tab = mActionBarAdapter.getCurrentTab();
        if (count > 0 && !mActionBarAdapter.isSearchMode() && mActionBarAdapter.getCurrentTab() == TabState.ALL) {
            startVoiceService();
        } else {
            stopVoiceService();
        }
    ...
    public void onQueryContactsInfo(final ArrayList<String> audioNameList) {
        stopVoiceService();
        LogUtils.i(TAG, "[onQueryContactsInfo] [vcs] audioNameList size:" + audioNameList.size());
        mAudioNameList = audioNameList;
        ...
        // produce selection and selectionArgs
        ...
        for (int i = 0; i < nameListSize; i++) {
            ...
            // LogUtils.i(TAG, "[onQueryContactsInfo] [vcs] selectionArgs[" + i
            // +
            // "]:" + selectionArgs[i]);
        }
        LogUtils.i(TAG, "[onQueryContactsInfo] [vcs] selection:" + selection);
        ...
        Cursor originalCursor = resolver.query(uri, CONTACT_PROJECTION, selection, selectionArgs, "sort_key");
        // error handling
        if (originalCursor == null) {
            LogUtils.w(TAG, "[onQueryContactsInfo] cusur is null.");
            // without any contacts info for voice search results, reset contacts found flag
            onRefreshContactsList(null);
            return;
        }
