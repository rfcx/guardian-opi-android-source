...
import com.android.launcher3.WallpaperPickerActivity.CropImageTask;
...
public class SavedWallpaperImages extends BaseAdapter implements ListAdapter {
    ...
    protected static class FileCropImageTask extends CropImageTask<Void, Void, BitmapRegionTileSource> {
        WallpaperPickerActivity mA;
        View mView;
        int mDbId;

        public FileCropImageTask(WallpaperPickerActivity a, View v, int dbId) {
            super();
            mA = a;
            mView = v;
            mDbId = dbId;
        ...
        protected BitmapRegionTileSource doInBackground(Void... params) {
            Log.i(TAG, "FileCropImageTask: doInBackground");
            String imageFilename = mA.getSavedImages().getImageFilename(mDbId);
            File file = new File(mA.getFilesDir(), imageFilename);
            int rotation = WallpaperCropActivity.getRotationFromExif(file.getAbsolutePath());
            return new BitmapRegionTileSource(mA, file.getAbsolutePath(), 1024, rotation);
        }

        @Override
        protected void onPreExecute() {
            Log.i(TAG, "FileCropImageTask: onPreExecute");
            mA.setWallpaperEnabled(false);
        }

        @Override
        protected void onPostExecute(BitmapRegionTileSource result) {
            Log.i(TAG, "FileCropImageTask: onPostExecute");
    ...
    public static class SavedWallpaperTile extends WallpaperPickerActivity.WallpaperTileInfo {
        ...
        public void onClick(WallpaperPickerActivity a) {
            Log.i(TAG, "SavedWallpaperTile: onClick, async mode = " + a.isAsyncTaskMode());
            if (a.isAsyncTaskMode() == true) {
                FileCropImageTask task = new FileCropImageTask(a, mView, mDbId);
