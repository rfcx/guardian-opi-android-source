    ...
    public boolean updateStatus(final ConversationCursor cursor, final boolean currentFooterShown) {
        if (cursor == null) {
            LogUtils.d(LogTag.getLogTag(),
                    "Entet the case that cursor is null, so display loading!!!");
        ...
        /** M: The current cursor state is changed or not with last. @{ */
        boolean cursorStatusChanged = (cursorStatus != mCursorState);
        mCursorState = cursorStatus;
        /** @} */
        ...
        if (UIProvider.CursorStatus.isWaitingForResults(cursorStatus)) {
            ...
            LogUtils.d(LogTag.getLogTag(),
                    " LoadMore finished folder [%s], totalCount %d, cursor count %d, cursorStatusChanged %s, allMessagesLoadFinish %s, currentFooterShown %s",
                    mLoadMoreUri, totalCount, cursor.getCount(), cursorStatusChanged, allMessagesLoadFinish, currentFooterShown);
            if (allMessagesLoadFinish) {
                /// M: When all messages load finish, load more do not need to display.
                showFooter = false;
            } else if (cursorStatusChanged) {
                /// M: When cursor state changed, to update loading status.
                updateLoadingStatus(false);
            } else {
                /// M: return the original state
                showFooter = currentFooterShown;
            }
        } else {
            showFooter = false;
            LogUtils.d(LogTag.getLogTag(), "Entet the case and footer view's shown state is : %s", showFooter);
        }
        return showFooter;
    }
