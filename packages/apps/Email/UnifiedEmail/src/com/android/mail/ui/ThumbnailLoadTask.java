...
public class ThumbnailLoadTask extends AsyncTask<Uri, Void, Bitmap> {
    ...
    public static ThumbnailLoadTask setupThumbnailPreview(
        ...
        /// M: We should just return if attachment is null or holder's w/h = 0/0, cause nothing gotta
        // be shown in these cases @{
        if (attachment == null || width == 0 || height == 0) {
            LogUtils.d(LOG_TAG, "Don't setupThumbnailPreview cause attachment : [%s], holder w/h: [%d/%d]",
