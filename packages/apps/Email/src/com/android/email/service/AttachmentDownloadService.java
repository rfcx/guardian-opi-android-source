...
public class AttachmentDownloadService extends Service implements Runnable {
    ...
    /*package*/ class DownloadSet extends TreeSet<DownloadRequest> {
        ...
        /**
         * M: Synchronize operations on mDownloadSet to avoid concurrent modification
         * @param req request need to be canceled
         */
        private synchronized void cancelDownload(DownloadRequest req) {
    ...
    /**
     * M: Synchronize the method to avoid concurrent modification on mDownloadSet
     * @param attachmentId id of dequeued attachment
     * @return
     */
    /*package*/ synchronized boolean dequeue(long attachmentId) {
        DownloadRequest req = mDownloadSet.findDownloadRequest(attachmentId);
        if (req != null) {
            if (LogUtils.isLoggable(TAG, LogUtils.DEBUG)) {
