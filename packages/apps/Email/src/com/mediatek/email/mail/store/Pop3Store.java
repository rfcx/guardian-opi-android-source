...
public class Pop3Store extends Store {
    ...
    /** M: The Connection pool */
    /// M: Temporarily give up to use connection pool for POP.
//    private final ConcurrentLinkedQueue<Pop3Connection> mConnectionPool =
//            new ConcurrentLinkedQueue<Pop3Connection>();
    ...
    /**
     * M: Gets a connection if one is available from the pool synchronously, or creates a new one if not.
     */
    // CHECKSTYLE:OFF
    synchronized Pop3Connection getConnectionSync() {
        // A fact had been found that for certain POP3 servers, reuse a connection to
        // send "STATUS" command, server would not respond correctly. So temporarily
        // give up to use connection pool for POP.
           return new Pop3Connection(this, mUsername, mPassword);
/*
        Pop3Connection connection = null;
        while ((connection = mConnectionPool.poll()) != null) {
            try {
                connection.setStore(this, mUsername, mPassword);
                // Make sure the connection is alive
                String response = connection.executeSimpleCommand("NOOP");
                if (!TextUtils.isEmpty(response) && '+' == response.charAt(0)) {
                    break;
                }
        ...
        if (connection == null) {
            connection = new Pop3Connection(this, mUsername, mPassword);
        }
        return connection;
*/
    }
    ...
    /**
     * M: Save a {@link Pop3Connection} in the pool for reuse synchronously. Any responses associated
     * with the connection are destroyed before adding the connection to the pool.
     */
    /// M: Temporarily give up to use connection pool for POP.
//    synchronized void poolConnectionSync(Pop3Connection connection) {
//        if (connection != null) {
