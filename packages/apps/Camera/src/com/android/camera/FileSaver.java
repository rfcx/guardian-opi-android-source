...
public class FileSaver{ //push thumbnail to thumbnail manager
    ...
    private FileSaverService.FileSaverListener mListener = new FileSaverService.FileSaverListener() {
    	...
    	public void onQueueStatus(boolean full) {
    	    if (mContext.getShutterManager() != null) {
    	        Log.i(TAG , "onQueueStatus , will setPhotoShutterEnabled : " + !full);
    ...
    private void addSaveRequest(SaveRequest r) {
        synchronized(this) {
            // some time onDestroy() execute before add save request which is through 
            // post runnable
            synchronized (mSaveServiceObject) {
                if (mSaverService == null) {
                    return ;
    ...
    // sychronized this method to avoid when mFileNamer is just construct by an AsynTask but
    // another AsynTask quickly get element from mFileNamer, it will return null.
    private synchronized String createName(int fileType, long dateTaken, int count,int tag) {
        if (mFileNamer == null) {
