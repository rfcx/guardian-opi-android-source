...
public class ModeChecker {
	...
	MATRIX_NORMAL_ENABLE[ModePicker.MODE_FACEART]       = new boolean[]{true, true}; //gangyun tech add
	...
	MATRIX_PREVIEW3D_ENABLE[ModePicker.MODE_FACEART]    = new boolean[]{true, false}; //gangyun tech add
	...
	MATRIX_SINGLE3D_ENABLE[ModePicker.MODE_FACEART]     = new boolean[]{true, false}; //gangyun tech add
    }
    ...
    if(FeatureOption.RY_GANGYUN_CAMERA_BEAUTY)
	{
    ...
    public static boolean getModePickerVisible(Camera camera, int cameraId, int mode) {
        ...
        //Add by liyu 2014.11.25
        if(camera.getApplicationContext().getResources().getBoolean(R.bool.config_hide_face_beauty)){
        	if(mode == ModePicker.MODE_FACE_BEAUTY){
        		visible = false;
        	}
        }
        //Add end
