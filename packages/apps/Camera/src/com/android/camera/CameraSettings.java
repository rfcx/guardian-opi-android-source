...
//
    ...
    private static List<String> buildPreviewRatios(Context context, Parameters parameters) {
	    ...
	    //changed for default radio start
		/*
		if(context.getResources().getBoolean(R.bool.config_modified_camera_values)){
	...
	if ((row == SettingChecker.ROW_SETTING_PICTURE_RATIO)	
		&& (mContext.getResources().getBoolean(R.bool.config_modified_camera_values)) && mCameraId != mContext.getResources().getInteger(R.integer.config_changeto_4_3_cameraId)) {
		String value = pref.getValue();
		Log.v(TAG, "resetIfInvalid(" + value + ")first= " + resetFirst);

		if (pref.findIndexOfValue(value) == NOT_FOUND) {
			if (resetFirst) {
				pref.setValueIndex(mContext.getResources().getInteger(R.integer.config_camera_type));
	...
	//added for default redio start
	int frontCamera = CameraHolder.instance().getFrontCameraId();
	if (context.getResources().getBoolean(R.bool.config_modified_camera_values) && currentCameraId != context.getResources().getInteger(R.integer.config_changeto_4_3_cameraId)) {
		...
		String picture_size = context.getResources().getString(
				R.string.picture_size_main);
		

		if (CameraSettings.setCameraPictureSize(picture_size, supported,
				parameters, targetRatio, context))

		{
			...
			editor.putString(CameraSettings.KEY_PICTURE_SIZE, picture_size);

			editor.apply();

		}
	}
	//added for default redio end
    }
