...
public class EasCalendarSyncHandler extends EasSyncHandler {
    ...
    private void handleExceptionsToRecurrenceRules(final Serializer s, final Entity entity,
        ...
        /// M: It is much more saftly to check query result, before use it. it maybe null. @{
        Cursor event = mContentResolver.query(asSyncAdapter(Events.CONTENT_URI), null, ORIGINAL_EVENT_AND_CALENDAR,
                new String[] { serverId, calendarIdString }, null);
        if (event == null) {
            LogUtils.w(TAG,
                    "handleExceptionsToRecurrenceRules failed, query Event [%s] is null", calendarIdString);
            return;
        }
        final EntityIterator exIterator = EventsEntity.newEntityIterator(event, mContentResolver);
        /// @}
    ...
    protected void setUpsyncCommands(final Serializer s) throws IOException {
        ...
        /// M: It is much more saftly to check query result, before use it. it maybe null. @{
        Cursor events = mContentResolver.query(asSyncAdapter(Events.CONTENT_URI),null,
                DIRTY_OR_MARKED_TOP_LEVEL_IN_CALENDAR, calendarIdArgument, null);
        if (events == null) {
            LogUtils.w(TAG, "setUpsyncCommands failed, query Calendar's Events [%s] is null", calendarIdString);
            return;
        }

        // Now go through dirty/marked top-level events and send them back to the server
        final EntityIterator eventIterator = EventsEntity.newEntityIterator(events, mContentResolver);
        /// @}
