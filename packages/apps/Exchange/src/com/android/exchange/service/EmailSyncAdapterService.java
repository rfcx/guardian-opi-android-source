...
public class EmailSyncAdapterService extends AbstractSyncAdapterService {
    ...
    /** M: Variables using by ping retry mechanism @{ */
    private static final long SECONDS = 1000;
    private static final long MINUTES = 60 * SECONDS;
    private static final long HOLD_DELAY_MINIMUM = 15 * SECONDS;
    private static final long HOLD_DELAY_MAXIMUM = 8 * MINUTES;
    private static final long PROBE_PING_SPAN = 60 * MINUTES;
    private static final HashMap<Long, Long> mPingHoldDelays = new HashMap<Long, Long>();
    private static final HashMap<Long, PendingIntent> mPendingIntents = new HashMap<Long, PendingIntent>();
    /** @} */
    ...
    public class SyncHandlerSynchronizer {
        ...
        public synchronized void modifyPing(final boolean lastSyncHadError,
            ...
            /// M: Clear the ping alarm(if any) for this account
            clearPingAlarm(account.mId);
            ...
            if (pushNeeded) {
                ...
                if (pingSyncHandler != null) {
                    ...
                    if (lastSyncHadError) {
                        /// M: Postpone sending ping by the retry mechanism
                        LogUtils.d(TAG, "modifyPing--lastSyncHadError");
                        setPingAlarm(account.mId, amAccount);
                    } else {
        ...
        public synchronized void pingComplete(final android.accounts.Account amAccount,
            ...
            if (pingStatus == EasOperation.RESULT_REQUEST_FAILURE ||
                ...
                LogUtils.d(TAG, "ping failed, start the retry mechanism");
                /// M: There's a scenario that the network is connected but the ping will constantly fail
                 // due to the APN/Gateway issue. Thus ought to carry on an escalated delaying retry process.
                setPingAlarm(accountId, amAccount);
            } else {
                /// M: Reset the account's ping delay time after current ping successful.
                mPingHoldDelays.put(accountId, HOLD_DELAY_MINIMUM);
                LogUtils.i("powertuneeas", "pingComplete stopServiceIfNoPings");
                stopServiceIfNoPings();
            }
        ...
        /**
         * M: Clear the ping alarm for the specific account
         * @param accountId
         */
        private void clearPingAlarm(long accountId) {
            synchronized (mPendingIntents) {
                PendingIntent pi = mPendingIntents.get(accountId);
                if (pi != null) {
                    AlarmManager alarmManager = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
                    alarmManager.cancel(pi);
                    LogUtils.d(TAG, "ping alarm cleared for " + accountId);
                    mPendingIntents.remove(accountId);
                }
            }
        }

        /**
         * M: Set an alarm for retring ping
         * @param accountId
         * @param amAccount
         */
        private void setPingAlarm(long accountId, android.accounts.Account amAccount) {
            synchronized (mPendingIntents) {
                PendingIntent pi = mPendingIntents.get(accountId);
                if (pi == null) {
                    if (!mPingHoldDelays.containsKey(accountId)) {
                        mPingHoldDelays.put(accountId, HOLD_DELAY_MINIMUM);
                    }
                    long pingHoldDelay = mPingHoldDelays.get(accountId);
                    if (pingHoldDelay > HOLD_DELAY_MAXIMUM) {
                        // It is risk to constrain any further ping. Just try
                        // a ping in every 1 hour is prefer
                        pingHoldDelay = PROBE_PING_SPAN;
                    } else {
                        mPingHoldDelays.put(accountId, pingHoldDelay * 2);
                    }

                    Context context = ((Context)EmailSyncAdapterService.this);
                    ...
                    pi = PendingIntent.getService(EmailSyncAdapterService.this, 0, intent, PendingIntent.FLAG_ONE_SHOT);

                    mPendingIntents.put(accountId, pi);
                    LogUtils.d(TAG, "set ping alarm for " + accountId + " as " + pingHoldDelay + " milliseconds");
                    AlarmManager alarmManager = (AlarmManager)getSystemService(Context.ALARM_SERVICE);
                    alarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + pingHoldDelay, pi);
                }
            }
        }
    }
