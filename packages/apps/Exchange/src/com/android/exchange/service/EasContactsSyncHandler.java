...
public class EasContactsSyncHandler extends EasSyncHandler {
    ...
    protected void setUpsyncCommands(final Serializer s) throws IOException {
        ...
        Cursor contacts = mContentResolver.query(uri, null,
                ContactsContract.RawContacts.DIRTY + "=1", null, null);
        if (contacts == null) {
            // Not do unsync operation since query result is null,
            // some exception happend during query ContactsProvider, Let's sync it next time.
            LogUtils.w(TAG, "setUpsyncCommands failed, query Contacts [%s] is null", uri);
            return;
        }

        // Get them all atomically
        final EntityIterator ei = ContactsContract.RawContacts.newEntityIterator(contacts);
        /// @}
