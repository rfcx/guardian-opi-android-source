...
//                /// @}
        ...
        private Contact getContactInfoForEmailAddress(String email) {
            ...
            if (cursor != null) {
                /// M:
                MmsLog.d(TAG, "getContactInfoForEmailAddress(): cursor != null, cursor.getCount()=" + cursor.getCount());
                try {
                    while (cursor.moveToNext()) {
                        boolean found = false;
                        synchronized (entry) {
                        ...
                        /// M:
                        MmsLog.d(TAG, "getContactInfoForEmailAddress(): found=" + found);
                        if (found) {
    ...
    public synchronized Drawable getGroupAvatar(final Context context, final long threadId) {
        MmsLog.d(M_TAG, "Contact.getGroupAvatar(): mAvatar is null ?= " + (mAvatar == null));
        if (mAvatar == null) {
            final Object lock = new Object();
            mIpMessageAvatarBitmap = null;
            sContactCache.pushTask(new Runnable() {
            ...
            MmsLog.d(M_TAG, "Contact.getGroupAvatar(): mIpMessageAvatarBitmap is null ?= " + (mIpMessageAvatarBitmap == null));
            if (null != mIpMessageAvatarBitmap) {
