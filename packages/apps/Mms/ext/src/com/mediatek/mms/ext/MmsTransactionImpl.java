        ...
        private boolean isPdpConnected() {
            ...
            if (connMgr != null) {
                try {
                    ...
                    MmsLog.d(TAG, "InterruptedException for sleep 1s");
                }

                NetworkInfo ni = connMgr.getNetworkInfo(ConnectivityManager.TYPE_MOBILE_MMS);
                if (ni != null && ni.isConnected()) {
                    MmsLog.d(TAG, "connected");
                    return true;
                } else {
                    if (ni == null) {
                        MmsLog.d(TAG, "getNetworkInfo fail");
                    } else {
                        MmsLog.d(TAG, "ni is not connected");
                    }
                    return false;
                }
            } else {
                MmsLog.d(TAG, "get ConnectivityManager failed!");
                return false;
            }
        }
    }
