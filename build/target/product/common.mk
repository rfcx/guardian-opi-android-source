...
#------------------RUNYEE CONFIG START------------------------------
ifneq (,$(strip $(RUNYEE_LOCALIZATION_OVERLAY)))
    PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/$(RUNYEE_LOCALIZATION_OVERLAY)
endif
ifneq (,$(strip $(RUNYEE_STYLE_OVERLAY)))
    PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/$(RUNYEE_STYLE_OVERLAY)
endif
ifeq (yes,$(strip $(RUNYEE_SETTINGS_OPPO)))
  PRODUCT_PACKAGES += Settingsoppo
  PRODUCT_PACKAGES += SchedulePowerOnOff-oppo
  
endif
ifeq (yes,$(strip $(RUNYEE_FINAL_FANTASY_CLOCK_WIDGET)))
  PRODUCT_PACKAGES += FinalFantasyClockWidget
endif

ifeq (yes,$(strip $(RUNYEE_FREE_MUSIC_WIDGET)))
  PRODUCT_PACKAGES += FreeMusicWidget
endif

ifeq (yes,$(strip $(RUNYEE_MEMORY_DISPLAY_WIDGET)))
  PRODUCT_PACKAGES += MemoryDisplayWidget
endif

ifeq ($(strip $(RUNYEE_CONFIG_MAGIC_ANALOG_CLOCK)), yes)
    PRODUCT_PACKAGES += MagicAnalogClockWidget
endif

ifeq (yes,$(strip $(RUNYEE_LAUNCHER2_ORIGINAL)))
  PRODUCT_PACKAGES += LauncherOriginal
endif

ifeq ($(strip $(RUNYEE_CONFIG_STYLE_MANAGER)), yes)
    PRODUCT_PACKAGES += StyleManager
endif

ifeq ($(strip $(RUNYEE_CONFIG_LAUNCHER2_MIUI)), yes)
    PRODUCT_PACKAGES += LauncherMiui
endif
ifeq ($(strip $(RUNYEE_CONFIG_LAUNCHER3)), yes)
	    PRODUCT_PACKAGES += Launcher3Original
endif

ifeq ($(strip $(RUNYEE_CONFIG_LAUNCHER2)), yes)
    PRODUCT_PACKAGES += Launcher2
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_UI_HUAWEI)))
  PRODUCT_PACKAGES += SystemUI_huawei
  PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/systemui-huawei
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_UI)))
  PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/systemui
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_UI_IOS)))
  PRODUCT_PACKAGES += SystemUI_ios
  PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/systemui-ios
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_UI_TF)))
  PRODUCT_PACKAGES += TFSystemUI
  PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/systemui-tf
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_UI_TF_NEW)))
  PRODUCT_PACKAGES += TFNewSystemUI
  PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/systemui-tf
endif

ifeq ($(strip $(RUNYEE_CONFIG_TORCH)), yes)
    PRODUCT_PACKAGES += Torch
endif

ifeq (yes,$(strip $(RUNYEE_SYSTEM_CLEAR)))
  PRODUCT_PACKAGES += SystemClear
endif

ifeq (yes,$(strip $(RUNYEE_DROP_VIDEO)))
  PRODUCT_PACKAGES += VideoInWin
endif

ifeq ($(HCT_VIDEO_DROPZONE),yes)
  PRODUCT_PACKAGES += VideoInWin
endif

ifeq ($(strip $(RUNYEE_CONFIG_OPPO_DIALER)), yes)
    PRODUCT_PACKAGES += RunyeeContacts
    PRODUCT_PACKAGES += RunyeeTeleService
    PRODUCT_PACKAGES += RunyeeDialer
endif

ifeq ($(strip $(RUNYEE_CONFIG_TF_DIALER)), yes)
    PRODUCT_PACKAGES += TFContacts
    PRODUCT_PACKAGES += RunyeeTeleService
    PRODUCT_PACKAGES += TFDialer
    PRODUCT_PACKAGE_OVERLAYS += device/runyee/optional/localization/styles/tfincallui
endif

ifeq ($(strip $(RUNYEE_MIUI_NOTE)), yes)
    PRODUCT_PACKAGES += MiUiNote
endif

ifeq ($(strip $(RUNYEE_CONFIG_MIUI_CALCULATOR)), yes)
    PRODUCT_PACKAGES += MiUiCalculatorv5
endif

ifeq ($(strip $(RUNYEE_MIUI_FileExplorer)), yes)
    PRODUCT_PACKAGES += MiUiFileExplorer
endif

ifeq ($(strip $(RUNYEE_COOEE_WEATHER_WIDGET)), yes)
    PRODUCT_PACKAGES += CooeeWeatherWidget
endif

ifeq ($(strip $(RUNYEE_OPPO_SOUND_RECORDER)), yes)
    PRODUCT_PACKAGES += OppoSoundRecorder
endif

ifeq ($(strip $(RUNYEE_OPPO_MUSIC)), yes)
    PRODUCT_PACKAGES += OppoMusic
endif

ifeq ($(strip $(RUNYEE_OPPO_CLOCK)), yes)
    PRODUCT_PACKAGES += OppoDeskClock
endif

ifeq ($(strip $(RUNYEE_MIUI_CLOCK)), yes)
    PRODUCT_PACKAGES += MiUiDeskClock
endif

ifeq ($(strip $(RUNYEE_MIUI_FM)), yes)
    PRODUCT_PACKAGES += MiUiV5FMRadio
endif

ifneq (,$(strip $(RUNYEE_BUILD_PRODUCT_NAME)))
	PRODUCT_PROPERTY_OVERRIDES += \
		ro.custom.build.product=$(RUNYEE_BUILD_PRODUCT_NAME)
endif

ifeq ($(strip $(RY_GANGYUN_CAMERA_BEAUTY)), yes)
    PRODUCT_PACKAGES += GanYunMakeup
endif

ifeq ($(strip $(RUNYEE_CONFIG_EASYTOUCH)), yes)
    PRODUCT_PACKAGES += EasyTouch
endif
...
ifeq ($(strip $(MTK_EXTERNAL_DONGLE_SUPPORT)), yes)
  ...
  $(call inherit-product-if-exists, mediatek/hardware/dongle-ril/vendorSoFile/vendorSoFile.mk)
...
#endif
ifeq ($(strip $(MTK_THEMEMANAGER_APP)), yes)
ifneq ($(strip $(HCT_MULTI_THEMEMANAGE)), yes)
  PRODUCT_PACKAGES += theme-res-hct1 \
                      theme-res-hct2 \
                      theme-res-hct3 \
                      libtinyxml
endif
...
#hct theme begin
# HCT_THEME_BBK ：尚品视野(vivo)
# HCT_MULTI_CIRCLE: 晶莹剔透
# HCT_THEME_DREAM：梦幻涟漪
# HCT_THEME_DREAM_COLOR：尚彩
# HCT_THEME_HSJZ：海上精致
# HCT_THEME_IOS：扁平极简
# HCT_THEME_JZY：静之韵
# HCT_THEME_LEWA： 精致荷语(乐蛙)
# HCT_THEME_QQ： 炫彩QQ糖
# HCT_THEME_SAMSUNG：经典商务(三星)
# HCT_THEME_TEXTURE_BUCKLE：质感纽扣
# HCT_THEME_TMXL： 透明炫亮
# HCT_THEME_WENLI ：质地纹理
# HCT_THEME_WENXINCIRCLE：温馨圆
# HCT_THEME_XIAOMI:清新简约(小米)
#hct theme end
ifeq ($(strip $(HCT_MULTI_THEMEMANAGE)), yes)
    PRODUCT_PACKAGES += StyleManager
    ifeq ($(strip $(HCT_THEME_BBK)), yes)
    	PRODUCT_PACKAGES += theme-res-bbk
    endif

    ifeq ($(strip $(HCT_MULTI_CIRCLE)), yes)
    	PRODUCT_PACKAGES += theme-res-circle
    endif

    ifeq ($(strip $(HCT_THEME_DREAM)), yes)
    	PRODUCT_PACKAGES += theme-res-dream
    endif

    ifeq ($(strip $(HCT_THEME_DREAM_COLOR)), yes)
    	PRODUCT_PACKAGES += theme-res-dreamcolor
    endif

    ifeq ($(strip $(HCT_THEME_HSJZ)), yes)
        PRODUCT_PACKAGES += theme-res-hsjz
    endif

    ifeq ($(strip $(HCT_THEME_IOS)), yes)
    	PRODUCT_PACKAGES += theme-res-ios7
    endif

    ifeq ($(strip $(HCT_THEME_JZY)), yes)
    	PRODUCT_PACKAGES += theme-res-jzy
    endif

    ifeq ($(strip $(HCT_THEME_LEWA)), yes)
    	PRODUCT_PACKAGES += theme-res-lewa
    endif

    ifeq ($(strip $(HCT_THEME_QQ)), yes)
    	PRODUCT_PACKAGES += theme-res-qq
    endif

    ifeq ($(strip $(HCT_THEME_SAMSUNG)), yes)
    	PRODUCT_PACKAGES += theme-res-samsung
    endif

    ifeq ($(strip $(HCT_THEME_TEXTURE_BUCKLE)), yes)
    	PRODUCT_PACKAGES += theme-res-texturebuckle
    endif

    ifeq ($(strip $(HCT_THEME_TMXL)), yes)
    	PRODUCT_PACKAGES += theme-res-tmxl
    endif

    ifeq ($(strip $(HCT_THEME_WENLI)), yes)
    	PRODUCT_PACKAGES += theme-res-wenli
    endif

    ifeq ($(strip $(HCT_THEME_WENXINCIRCLE)), yes)
    	PRODUCT_PACKAGES += theme-res-wenxincircle
    endif

    ifeq ($(strip $(HCT_THEME_XIAOMI)), yes)
    	PRODUCT_PACKAGES += theme-res-xiaomi
    endif
...
ifeq ($(strip $(HCT_MULTI_THEMEMANAGE)), yes)
	#====彩色=====
	ifeq ($(strip $(RUNYEE_THEME_COLOR)), yes)
	    PRODUCT_PACKAGES += theme-res-color
	endif
	#====新风尚====
	ifeq ($(strip $(RUNYEE_THEME_NEWFASHION)), yes)
	    PRODUCT_PACKAGES += theme-res-newfashion
	endif
	#====清新派====
	ifeq ($(strip $(RUNYEE_THEME_FLASHSTYLE)), yes)
	    PRODUCT_PACKAGES += theme-res-flashstyle
	endif
	#====品味====
	ifeq ($(strip $(RUNYEE_THEME_GRADE)), yes)
	    PRODUCT_PACKAGES += theme-res-grade
	endif
	#====锤子====
	ifeq ($(strip $(RUNYEE_THEME_HAMMER)), yes)
	    PRODUCT_PACKAGES += theme-res-hammer
	endif
	#====简爱====
	ifeq ($(strip $(RUNYEE_THEME_JANEEYRE)), yes)
	    PRODUCT_PACKAGES += theme-res-janeeyre
	endif
	#====四季鲜==
	ifeq ($(strip $(RUNYEE_THEME_SEASONS)), yes)
	    PRODUCT_PACKAGES += theme-res-seasons
	endif
	#====type====
	ifeq ($(strip $(RUNYEE_THEME_TYPE)), yes)
	    PRODUCT_PACKAGES += theme-res-type
	endif
        #====oppo====
	ifeq ($(strip $(RUNYEE_THEME_OPPO)), yes)
	    PRODUCT_PACKAGES += theme-res-oppo
	endif
endif
#-------------------------------------主题end---------------------------------
...
#==============GMS start====================================
ifeq (yes,$(strip $(RY_GOOGLE_GMS_NEW_R4)))
  PRODUCT_PROPERTY_OVERRIDES += \
    ro.com.google.clientidbase=$(eval RY_GOOGLE_CLIENTIDBASE ?= android)$(RY_GOOGLE_CLIENTIDBASE) \
    ro.com.google.clientidbase.ms=$(eval RY_GOOGLE_CLIENTIDBASE_MS ?= android)$(RY_GOOGLE_CLIENTIDBASE_MS) \
    ro.com.google.clientidbase.am=$(eval RY_GOOGLE_CLIENTIDBASE_AM ?= android)$(RY_GOOGLE_CLIENTIDBASE_AM) \
    ro.com.google.clientidbase.gmm=$(eval RY_GOOGLE_CLIENTIDBASE_GMM ?= android)$(RY_GOOGLE_CLIENTIDBASE_GMM) \
    ro.com.google.clientidbase.yt=$(eval RY_GOOGLE_CLIENTIDBASE_YT ?= android)$(RY_GOOGLE_CLIENTIDBASE_YT)
endif

ifeq (yes,$(strip $(RY_GOOGLE_GMS_NEW_R4)))
  $(call inherit-product, vendor/google_new_r4/products/gms.mk)
endif
#==============GMS end======================================
...
#lwj
ifeq ($(strip $(RUNYEE_PTT)),yes)
  PRODUCT_PACKAGES += PTT
endif
...
#lwj add for runyee-apk begin
ifeq ($(strip $(RY_MobileSafe360)),yes)
  PRODUCT_PACKAGES += MobileSafe360
endif

ifeq ($(strip $(RY_MobileAssitant360)),yes)
  PRODUCT_PACKAGES += MobileAssitant360
endif

ifeq ($(strip $(RY_BaiduSearch)),yes)
  PRODUCT_PACKAGES += BaiduSearch
endif

ifeq ($(strip $(RY_BaiduBrowser)),yes)
  PRODUCT_PACKAGES += BaiduBrowser
endif
#lwj add for runyee-apk end
